...
#        self.final_proj1 = nn.Conv1d(hidden_channels, out_channels, 1) 
        self.final_proj = FastKANLayer(hidden_channels, out_channels)
    ...
    def forward(self, x, mask, mu, t, spks=None, cond=None):
        ...
        output = self.final_proj((x * mask).permute(0,2,1))
        output = output.permute(0,2,1)
#        print('kan: ', output.shape, mask.shape)
#        output = self.final_proj1(x*mask)
...
class AttentionWithFastKAN(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = FastKANLayer(dim, dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
